...
function addInputField(t) {
    ...
    if (count == limits)
        ...
        e.innerHTML = "<td><input autocomplete='off' type='text' name='customer_name' onkeypress='invoice_customerList(" + count + ");' class='form-control productSelection common_product' placeholder='Customer Name' id='" + a + "' required tabindex='" + tab1 + "'><input type='hidden' class='common_product autocomplete_hidden_value  customer_id_" + count + "' name='customer_id[]' id='SchoolHiddenId'/></td> <td><input autocomplete='off' type='text' name='category_name' onkeypress='invoice_categoryList(" + count + ");' class='form-control productSelection common_product' placeholder='Category Name' id='" + a + "' required tabindex='" + tab1 + "'><input type='hidden' class='common_product autocomplete_hidden_category_value  category_id_" + count + "' name='category_id[]' id='SchoolHiddenId'/></td> <td> <input type='text' name='discount[]' value='0' required='required'  id='total_qntt_" + count + "' class='common_qnt total_qntt_" + count + " form-control text-right'  placeholder='0.00' min='0' tabindex='" + tab3 + "'/></td><td><button tabindex='" + tab5 + "' style='text-align: center;' class='btn btn-danger' type='button' value='Delete' onclick='deleteRow(this)'><i class='fa fa-close'></i></button></td>",
...
"use strict";
function invoice_customerList(sl) {
    ...
    var options = {
        minLength: 0,
        source: function( request, response ) {
            var customer_name = $('#customer_name_'+sl).val();
            $.ajax( {
                url: base_url + "Cdiscount/autocompletecustomersearch",
                method: 'post',
                dataType: "json",
                data: {
                    term: request.term,
...
"use strict";
function invoice_categoryList(sl) {
    ...
    var options = {
        ...
        source: function( request, response ) {
            ...
            $.ajax( {
                url: base_url + "Cdiscount/autocompletecategorysearch",
                method: 'post',
                dataType: "json",
                data: {
                    term: request.term,
        ...
        select: function( event, ui ) {
            $(this).parent().parent().find(".autocomplete_hidden_category_value").val(ui.item.value);
            $(this).val(ui.item.label);
            var id=ui.item.value;
            //var dataString = 'pro duct_id='+ id;
            //var base_url = $('.baseUrl').val();
