...
function addInputField(t) {
  ...
  if (count == limits)
    ...
    (e.innerHTML =
        ...
        count +
        "' name='product_id[]' id='SchoolHiddenId'/></td> <td><input type='text' name='available_quantity[]' id='' class='form-control text-right common_avail_qnt available_quantity_" +
        count +
        "' value='0' readonly='readonly' /></td><td><input class='form-control text-right common_name unit_" +
        count +
        " valid' value='None' readonly='' aria-invalid='false' type='text'></td><td> <input type='text' name='product_quantity[]' value='1'  onkeyup='quantity_calculate(" +
        count +
        ...
        tab3 +
        "'/></td><td><input type='text' name='product_rate[]' onkeyup='quantity_calculate(" +
        count +
        ");' onchange='quantity_calculate(" +
        count +
        ");' id='price_item_" +
        ...
        " form-control text-right'  placeholder='0.00' min='0' tabindex='" +
        tab4 +
        "'/> <input type='hidden' name='supplier_price[]'' id='supplier_price_" +
        count +
...
function quantity_calculate(item) {
  ...
  var item_discount = discount ? n * (parseFloat(discount) / 100) : 0;
  //   var c = quantity * price_item * total_tax;
  $("#total_price_" + item).val(n - item_discount);
  // $("#all_tax_" + item).val(c);
