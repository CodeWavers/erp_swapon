...
function add_pay_row(sl) {
    ...
    pay_div.append(
        ...
        "</div>" +
        '<div class="col-sm-1">' +
        '<a id="add_pt_btn" onclick="add_pay_row(' +
        sl +
...
function bank_paymet(val, sl) {
    if (val == 2 || 3 || 4 || 5 || 6) {
        ...
        if (val == 6) {
            var style = "block";
            document.getElementById("tt_id_" + sl).setAttribute("required", true);
        } else {
            var style = "none";
            document.getElementById("tt_id_" + sl).removeAttribute("required");
        }

        document.getElementById("tt_div_" + sl).style.display = style;
    }
}
