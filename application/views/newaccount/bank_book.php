...
if (isset($_POST['btnSave'])) {
    ...
    $sql = $this->accounts_model->bankbook_firstqury($FromDate, $HeadCode, $outlet_id);
    ...
    $sql = $this->accounts_model->bankbook_secondqury($FromDate, $HeadCode, $ToDate, $outlet_id);
    $oResult = $oAccount->SqlQuery($sql);

    // echo '<pre>';
    // // print_r($oResult->rows);
    // // exit();

    foreach ($oResult->rows as $k => $v) {

        $tr_outlet = $this->Warehouse->get_outlet_by_user_id($oResult->rows[$k]["CreateBy"]);

        $oResult->rows[$k]["tr_outlet"] = $tr_outlet;

        if ((float) $v['Credit'] > 0) {
            $headAgainst = $this->accounts_model->get_the_head_aginst($v['VNo'], 'Debit');
            $oResult->rows[$k]["headAgainst"] = '';
            if ($headAgainst) {
                foreach ($headAgainst as $ha) {
                    $oResult->rows[$k]["headAgainst"] .= $ha['HeadName'] . ', ';
                }
            }
        }

        if ((float) $v['Debit'] > 0) {
            $headAgainst = $this->accounts_model->get_the_head_aginst($v['VNo'], 'Credit');
        ...
        $oResult->rows[$k]["headAgainst"] = substr($oResult->rows[$k]["headAgainst"], 0, -2);
    }
    // echo '<pre>';
    // print_r($oResult);
    // exit();
                        ...
                        <div class="row" id="">
                            ...
                            <div class="col-sm-6">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" name="dtpFromDate" value="" placeholder="<?php echo display('date') ?>" class="datepicker form-control" autocomplete="off">
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" name="dtpToDate" value="" placeholder="<?php echo display('date') ?>" class="datepicker form-control" autocomplete="off">
                                    </div>
                                </div>
                            ...
                            <div class="">
                                ...
                                <table width="100%" class="table table-stripped" cellpadding="1" cellspacing="1">
                                    ...
                                    <tr class="table_head">
                                        ...
                                        <td align="center"><b>Head Against</b></td>
                                    ...
                                    ?>
                                        <tr class="table_data">
                                            ...
                                            ?>
                                            <td align="center" bgcolor="<?php echo html_escape($bg); ?>"><?php echo html_escape($oResult->rows[$i]['tr_outlet']); ?></td>
                                            <td align="center" bgcolor="<?php echo html_escape($bg); ?>"><?php echo html_escape($oResult->rows[$i]['HeadName']); ?></td>
                                            <td align="center" bgcolor="<?php echo html_escape($bg); ?>"><?php echo html_escape($oResult->rows[$i]['headAgainst']); ?></td>
                                            <td align="center" bgcolor="<?php echo html_escape($bg); ?>"><?php echo html_escape($oResult->rows[$i]['Narration']); ?></td>
