...
class Lpurchase
{
    ...
    public function previous_stock_form()
    {
        $CI = &get_instance();
        $CI->load->model('Purchases');
        $CI->load->model('Web_settings');
        $all_supplier = $CI->Purchases->select_all_supplier();
        //$all_shelf = $CI->Purchases->select_all_shelf();
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
        $bank_list        = $CI->Web_settings->bank_list();
        $data = array(
        ...
        $purchaseForm = $CI->parser->parse('purchase/previous_purchase_form', $data, true);
        return $purchaseForm;
    }
