...
class Cretrun_m extends CI_Controller
{
    ...
    public function invoice_return_form_c($invoice_id)
    {
        $CI = &get_instance();
        ...
        if ($query->num_rows() == 0) {
            ...
            redirect('Ccourier/courier_status');
        }
        $CI = &get_instance();
        $CI->auth->check_admin_auth();
        $CI->load->library('lreturn');
    ...
    public function return_to_cw()
    {
        ...
        $rqsn_second_details = $CI->rqsn->get_recieved_item_rqsn($rqsn_id);

        if (!$rqsn_details) {
            $this->session->set_userdata(array('error_message' => "Please enter correct requisition no."));
            redirect('Cretrun_m');
        }

        if (!$rqsn_second_details) {
            $this->session->set_userdata(array('error_message' => "Requisition's products haven't been recieved yet."));
            redirect('Cretrun_m');
        }

        $data = array(
            "title" => "Return to CW",
            "rqsn_details"  => $rqsn_second_details
        );
        // print_r($data);
        // exit;


        $view = $CI->parser->parse('return/outlet_return_form', $data, true);
        $this->template->full_admin_html_view($view);
    }

    public function confirm_return_cw()
    {
        $checked = $this->input->post('rtn', true);
        $ret_qty = $this->input->post('return_qty', true);
        $details_id = $this->input->post('details_id', true);
        ...
        for ($i = 0; $i < count($checked); $i++) {
            if ($checked[$i] != 0) {
                ...
                $q = $this->db->select('a_qty')
                    ->from('rqsn_details')
                    ->where('rqsn_detail_id', $det)
                    ->get()
                    ->result_array();
                ...
                $this->db->where('rqsn_detail_id', $det);
                $this->db->update('rqsn_details', ['a_qty' => $new_qty]);
            }
        }
