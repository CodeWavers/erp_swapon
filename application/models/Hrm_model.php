...
//=========================== employeee data start ===========================
  ...
  $createdate=date('Y-m-d H:i:s');
   ...
   public function employee_editdata($id){
        $this->db->select('a.*');
        $this->db->from('employee_history a');
//       $this->db->join('employee_academic b','b.employee_id=a.employee_id');
//        $this->db->join('employee_tr c','c.employee_id=a.employee_id');
//        $this->db->join('employee_ex d','d.employee_id=a.employee_id');
      //  $this->db->group_by(array("b.degree", "c.tr_name","d.company"));
       // $this->db->group_by('b.degree','c.tr_name','d.company');
      //  $this->db->or_group_by('d.company');
   ...
   public function tr_editdata($id){
        $this->db->select('a.*');
        $this->db->from('employee_tr a');
        $this->db->where('a.employee_id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
   ...
   public function ac_editdata($id){
        $this->db->select('a.*');
        $this->db->from('employee_academic a');
        $this->db->where('a.employee_id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
   ...
   public function ex_editdata($id){
        $this->db->select('a.*');
        $this->db->from('employee_ex a');
        $this->db->where('a.employee_id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
...
// update employee
    ...
    //delete employee
      ...
      public function employee_details($id){
        $this->db->select('a.*,b.designation,c.*,d.*,e.*');
        $this->db->from('employee_history a');
        $this->db->join('designation b','a.designation = b.id');
        $this->db->join('employee_academic c','c.employee_id = a.employee_id');
        $this->db->join('employee_ex d','d.employee_id = a.employee_id');
        $this->db->join('employee_tr e','e.employee_id = a.employee_id');
        $this->db->where('a.employee_id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
